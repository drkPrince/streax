...
const CheckIn = ({habit, cleanDates, user}) => 
{
	...
	return (
		<div className=''>
            <motion.div className={`rounded-full bg-gray-700 w-16 h-7 flex items-center ${done ? 'justify-end' : 'justify-start'}`}>
                <motion.div layout transition={{type: 'spring', stiffness: 700, damping: 30}} className={`rounded-full w-10 h-10 flex justify-center items-center ${done ? 'bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500' : 'bg-gradient-to-r from-gray-200 via-gray-400 to-gray-600'}`} onClick={checkIn}>
