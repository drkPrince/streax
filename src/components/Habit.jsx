...
const Habit = ({habit, user}) => 
{
	...
	return (
		<motion.div className={`mb-6 shadow-md px-3 md:px-6 py-2 md:py-5 border-l-2 border-${habit.color}-dark rounded-sm`} variants={variant} style={{background: 'var(--sec-bg)'}}>
            <div className='flex items-center mb-1 md:mb-2'>
                <Link to={{pathname: `/${habit.id}`, state: {...habit, ...streak, userid: user.uid} }} >
                    <span className='text-lg md:text-2xl text-transparent bg-clip-text bg-gradient-to-r from-red-100 to-blue-100 bold'>
                        {habit.name}
                    </span>
                </Link>
                <div>
            ...
            <div className="grid gap-x-2 grid-cols-12">
                <div className={`grid grid-cols-6 sm:grid-cols-8 gap-x-2 col-span-10`}>
                    {dates}
                </div>
