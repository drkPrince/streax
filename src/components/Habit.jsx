...
const Habit = ({habit, user}) => 
{
	...
	return (
		<motion.div className={`mb-6 shadow-md px-3 md:px-6 py-2 md:py-5 border-l-2 border-${habit.color}-dark rounded-sm relative`} variants={variant} style={{background: 'var(--sec-bg)'}}>
            <div className='flex justify-between items-center mb-1 md:mb-2'>
                <div className='flex items-center'>
                    <Link to={{pathname: `/${habit.id}`, state: {...habit, ...streak, userid: user.uid} }} >
                        <span className='text-lg md:text-2xl text-white bold hover:text-red-600 transition-colors duration-500'>
                            {habit.name}
                        </span>
                    </Link>
                    <div>
            ...
            <div className="grid gap-x-2 grid-cols-12">
                <div className={`grid grid-cols-6 sm:grid-cols-8 gap-x-2 col-span-12 sm:col-span-10`}>
                    {dates}
                </div>
...
const renderDates = (completedDates, color) => 
{
    ...
    const daysInterval = eachDayOfInterval({start: sub(today, {days: 7}), end: today})
    ...
    const dates = daysInterval.map(d => 
    {
    	...
    	const day = itsToday ? <h1 className='text-xs md:text-sm text-white mb-2'>Today</h1> : <h1 className='text-xs md:text-sm mb-2 text-gray-600'>{format(d, 'EE')}</h1>
