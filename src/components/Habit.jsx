...
const Habit = ({habit, user}) => 
{
	...
	return (
		<motion.div className={`mb-6 shadow-md px-6 py-5 border-l-2 border-${habit.color}-dark rounded-sm`} variants={variant} style={{background: '#060607'}}>
            <div className='flex justify-between  mb-2'>
                <Link to={{pathname: `/${habit.name}`, state: {...habit, ...streak, userid: user.uid} }} >
                    <span className='flex items-center text-2xl text-gray-200 bold'>
                        {habit.name}
                        <span>
                            {streak.currentStreak===streak.longestStreak ? 
                                <svg className="w-5 h-5 ml-2 text-yellow-50" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                : ''}
                        </span>
                    </span>
                </Link>
